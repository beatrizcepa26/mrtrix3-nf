process Response_Function_Estimation {
    ...
    if (params.rfe_algorithm == 'dhollander') {
        ...
        algorithm_params = tournier_opt
        output_files = "response.txt"
        command_line = "dwi2response tournier $data_mif $output_files $algorithm_params $dw_grad_opt $dwi2response_opt $python $standard_opt"

    } else {
        error "Unsupported RFE algorithm: ${params.rfe_algorithm}. Supported algorithms are: dhollander, fa, manual, msmt_5tt, tax, tournier."
    }
